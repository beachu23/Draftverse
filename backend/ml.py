...
def load_all() -> None:
    ...
    # Precomputed animal archetypes (generated by generate_archetypes.py)
    archetypes_path = DATA_DIR / "player_archetypes.json"
    if archetypes_path.exists():
        with open(archetypes_path) as f:
            state.archetypes = json.load(f)
    else:
        state.archetypes = {}
...
_PROSPECT_STR_COLS = {'slug', 'name', 'school', 'position'}


def load_prospects_2026() -> list[dict]:
    path = DATA_DIR / "prospects_2026.csv"
    if not path.exists():
        return []
    df = pd.read_csv(path)
    result = []
    for _, row in df.iterrows():
        ...
        for col in df.columns:
            ...
            if pd.isna(val):
                continue
            p[col] = str(val) if col in _PROSPECT_STR_COLS else float(val)
        result.append(p)
    return result
...
def player_to_dict(row: pd.Series) -> dict:
    ...
    archetype = state.archetypes.get(slug) if state.archetypes else None
    return {
